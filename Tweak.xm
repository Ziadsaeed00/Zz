...
// ==========================================
// 📢 دالة موحدة ومتوافقة تماماً لتخطي حظر keyWindow
// ==========================================
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (@available(iOS 13.0, *)) {
            for (id scene in [UIApplication sharedApplication].connectedScenes) {
                if ([[scene valueForKey:@"activationState"] integerValue] == 0) { // UISceneActivationStateForegroundActive
...
- (NSURLSessionDataTask *)dataTaskWithRequest:(NSURLRequest *)request completionHandler:(void (^)(NSData *data, NSURLResponse *response, NSError *error))completionHandler {
    ...
    // 1. فحص واعتراض سيرفر الإعلانات القديم وتحويله
    ...
    // 2. فحص واعتراض السيرفر الجديد مباشرة (هنا تم إصلاح الخطأ وتمرير الـ handler المخصص)
    if ([urlStr containsString:@"tn.maildisposable.com"] && completionHandler) {
        ...
        // تم استبدال الخيار القديم بـ customHandler لإنهاء تحذير المتغير غير المستخدم
